...
def log_training_results(
    ...
    swanlab.log({
        f'{order}/train/PSNR': psnr_val.item(),
        f'{order}/train/l2_loss': l2_loss.item(),
        f'{order}/train/bpp': bpp.item(),
        f'{order}/train/total_loss': loss.item(),
        f'{order}/train/psnr_bpp': psnr_val.item() / bpp.item(),
        f'{order}/train/temperature': temperature,
        f'{order}/train/lr': lr,
        f'{order}/train/aux_loss': aux_loss.item(),
...
def log_valid_results(
    ...
    try:
        swanlab.log({
            f'{order}/valid/PSNR': psnr_val,
            f'{order}/valid/PSNR_ref': psnr_val_ref,
            f'{order}/valid/bpp': bpp,
            f'{order}/valid/psnr_bpp': psnr_val / bpp,
        }, step=iteration)
    except Exception as e:
        # swanlab 未 init（无 active Run）或不可用时静默跳过，不影响 eval 出结果
        logger.debug(f'swanlab log skipped: {e}')