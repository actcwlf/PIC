...
def msssim(img1, img2, window_size=11, size_average=True, val_range=None, normalize=False, full=True):
    ...
    def forward(self, img1, img2):
        ...
        if channel == self.channel and self.window.dtype == img1.dtype:
            ...
            window = create_window(
                self.window_size,
                channel).to(img1.device, non_blocking=True).type(img1.dtype)
            self.window = window
...
class MSSSIM(torch.nn.Module):
    def __init__(self, window_size=11, size_average=True, channel=3, max_val=None):
        super(MSSSIM, self).__init__()
        self.window_size = window_size
        self.size_average = size_average
        self.channel = channel
        self.val_range = max_val

    def forward(self, img1, img2):
        # TODO: store window between calls if possible
        return msssim(
            img1, img2, window_size=self.window_size,
            size_average=self.size_average, val_range=self.val_range, full=True
            )
