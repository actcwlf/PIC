...
def train_model(
    ...
    if pp.entropy_checkpoint:
        (ckpt, train_meta_ckpt) = torch.load(pp.entropy_checkpoint)
        dct_params = {}
        for k, v in ckpt.items():
            if 'dct_layer.' in k:
                dct_params[k] = v
        print(dct_params.keys())
        model.load_state_dict(dct_params, strict=False)
    ...
    for k, v in model.named_parameters():
        if 'bottleneck.' in k:
            ...
            net[k] = v
    # print(net.keys())
    # print(entropy_net.keys())
    # exit()
    # optimizer = optim.Adam(model.parameters(), lr=op.lr)

    net_optimizer = optim.Adam(net.values(), lr=op.lr)
    entropy_optimizer = optim.Adam(entropy_net.values(), lr=op.entropy_lr)
    syn_optimizer = optim.Adam(syn_net.values(), lr=op.syn_lr)
    quantile_optimizer = optim.Adam(quantile_params.values(), lr=op.aux_lr)
    ...
    for epoch in range(start_epoch, op.total_epoch):
        for i, x in enumerate(train_loader):
            ...
            if train_meta.trained_iterations % pp.eval_freq == 0:
                ...
                eval_model(model, pp, epoch=epoch, step=i, total_iter=train_meta.trained_iterations)
