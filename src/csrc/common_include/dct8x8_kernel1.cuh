...
*/
// 矩阵构造公式
// F(u) = c(u) * \sum_{n=0}^{N-1} x[n]cos( (n + 0.5) * \pi * u / N),  c(0) = \sqrt{1/8}, c(u)=\sqrt{2/8}, u\neq 0,u\in \{0,\dots, 7\}, N=8
__constant__ float DCTv8matrix[] =
{   // sqrt(1/N), math.cos((0.5 * math.pi) /8) * math.sqrt(2/8)
...
__global__ void CUDAkernel1DCT(float *Dst, int ImgWidth, int OffsetXBlocks, int OffsetYBlocks)
{
    ...
    // printf("tx %d ty %d pix %f\n", tx, ty, CurBlockLocal1[(ty << BLOCK_SIZE_LOG2) + tx ]);
...
* \return None
*/
__global__ void CUDAkernel1DCTv2(float *Dst, cudaTextureObject_t tex, int ImgWidth, int OffsetXBlocks, int OffsetYBlocks)
{
    // Block index
    const int bx = blockIdx.x + OffsetXBlocks;
    const int by = blockIdx.y + OffsetYBlocks;
    ...
    //copy current image pixel to the first block
    CurBlockLocal1[(ty << BLOCK_SIZE_LOG2) + tx ] = tex2D<float>(tex, tex_x, tex_y);
