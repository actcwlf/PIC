...
std::shared_ptr<std::vector<llhuff::LLHuffmanEncoder::Symbol>>
    ...
    SYMBOL_TYPE* data, size_t size, const int max_codeword_length) {
    ...
    if(log2(freq.size()) > max_codeword_length) {
        std::cout << "exceed max_codeword_length\n";
        return nullptr;
    }
...
llhuff::LLHuffmanEncoder::get_encoder_table(
    ...
    constexpr int unit_type_bit_size = sizeof(UNIT_TYPE) * 8;
    table.compressed_size = (size + unit_type_bit_size - 1) / unit_type_bit_size;
...
void llhuff::LLHuffmanEncoder::encode_memory(UNIT_TYPE* out, size_t size_out,
    ...
    size_t i = 0;
    size_t diff = 0;
    while(i < size_out && in_unit < size_in) {
        auto code = encoder_table->dict[in[in_unit]];
        
        while(at + code.length < max_bits && in_unit < size_in) {
            window <<= code.length;
            // std::cerr << "code.length " << code.length << std::endl;
            window += code.codeword;
            // std::cout << "cw " << std::bitset<4>(code.codeword) << " code_Length " << code.length << std::endl;

            // std::cout << "tmp win " << std::bitset<32>(window) << std::endl;
        ...
        if(in_unit == size_in) { // 符号已经用完，但最后一个window没有填满
            window <<= (max_bits - at);
            out_ptr[i] = window;
            window = 0;
            break;
        ...
        // 此时at位置是可用的
        diff = at + code.length - max_bits;
        // std::cerr << "at " << at << " diff " << diff << std::endl;
        if (diff == 0) {
            // 下一个code刚好可以放在at位置
            window <<= code.length;
            // std::cerr << "code.length " << code.length << std::endl;
            window += code.codeword;
            // std::cout << "window " << std::bitset<32>(window) << std::endl;
            out_ptr[i] = window;
            i++;
            window = 0;
            at = 0;
    ...
    // std::cerr << "encode " << size_out << ' ' << i << ' ' << std::bitset<32>(window) << std::endl;
    if (window != 0) {
        out_ptr[i] = window << (max_bits - diff);
    }
}
...
llhuff::LLHuffmanEncoder::get_decoder_table(
    std::shared_ptr<llhuff::LLHuffmanEncoderTable> enc_table, const int max_codeword_length
) {

    std::shared_ptr<cuhd::CUHDCodetable> table
        = std::make_shared<cuhd::CUHDCodetable>(enc_table->dict.size(), max_codeword_length);
    
    // std::cerr << "enc_table->dict.size() " << enc_table->dict.size() << std::endl;
    ...
    for(auto &i: enc_table->dict) {    
        ...
        for(size_t j = 0; j < num_entries; ++j) {
            ...
            // std::cerr << "j " << j;
            // std::cerr << " codeword " << std::bitset<sizeof(int)>(codeword);
            // std::cerr << " length " << (int)length;
            // std::cerr << " symbol " << (int)symbol;
            // std::cerr << " idx " << (codeword << shift) + j ;
            dec_ptr[(codeword << shift) + j] = {length, symbol};
            // std::cerr << " dec_ptr " << (int)((codeword << shift) + j) << std::endl;
        }
    }

    cuhd::CUHDCompactCodeTableItem * compact_table = table->get_compact_table();
    int total_entries = 1 << max_codeword_length;

    // std::cerr << "total entries " << total_entries << " num entries " << table->get_num_entries() << std::endl;

    constexpr size_t entry_len = sizeof(SYMBOL_TYPE) + sizeof(BIT_COUNT_TYPE);
    static_assert(entry_len == 2, "Total bit len of SYMBOL_TYPE and BIT_COUNT_TYPE must be 16");


    const uint16_t * code_table = reinterpret_cast<uint16_t*>(dec_ptr); // 这里依赖 SYMBOL_TYPE SYMBOL_TYPE均为位长8
    uint16_t current_code = code_table[0];
    int item_idx = 0;
    for (int i = 0; i < total_entries; i++) {
        if(current_code != code_table[i]) {
            // std::cerr <<"i " << i << " item_idx " << item_idx << std::endl;
            compact_table[item_idx].cum_run_length = i;
            compact_table[item_idx].item = *(reinterpret_cast<cuhd::CUHDCodetableItemSingle*>(&current_code));
            current_code = code_table[i];
            

            // std::cout << "cum rl " << i;
            // std::cout << " bit num " <<  (int)compact_table[item_idx].item.num_bits;
            // std::cout << " symbol " <<  (int)compact_table[item_idx].item.symbol << std::endl;
            item_idx++;

        }
    }
    // std::cerr << "item_idx " << item_idx << std::endl;
    if(item_idx >= table->get_num_entries()) {
        std::cerr << "error item_idx " << item_idx << std::endl;
    }
    compact_table[item_idx].cum_run_length = total_entries;
    compact_table[item_idx].item = *(reinterpret_cast<cuhd::CUHDCodetableItemSingle*>(&current_code));
    // std::cout << "cum rl " << total_entries;
    // std::cout << " bit num " <<  (int)compact_table[item_idx].item.num_bits;
    // std::cout << " symbol " <<   (int)compact_table[item_idx].item.symbol << std::endl;
