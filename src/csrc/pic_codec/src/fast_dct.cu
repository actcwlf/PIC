...
__global__ void idct_kernel(float *src, int total_block, float scale)
{
    ...
    __shared__ float block[256 * 8]; // thread_per_block * BLOCK_SIZE


    #pragma unroll
    for (unsigned int i = 0; i < BLOCK_SIZE; i++)
        block[threadIdx.x * 8 + i] = src[gid * 8 + i] * scale;
    // __syncthreads();  // 这里似乎因为调度原因不需要显式同步

    //process rows
    CUDAsubroutineInplaceIDCTvector(block + threadIdx.x * 8, 1);
    // __syncthreads();  // 这里似乎因为调度原因不需要显式同步

    int block_id = threadIdx.x / 8;
    //process columns
    CUDAsubroutineInplaceIDCTvector(block + block_id * 64 + gid % 8, 8);


    // __syncthreads();  // 这里似乎因为调度原因不需要显式同步
    #pragma unroll
    for (unsigned int i = 0; i < BLOCK_SIZE; i++)
        src[gid * 8 + i] = block[threadIdx.x * 8 + i];
}
...
{   
    ...
    int blocks = (input.size(0) * 8 + thread_per_block - 1) / thread_per_block;
    idct_kernel<<<blocks, thread_per_block>>>(input.data_ptr<float>(), input.size(0) * 8, scale);
...
{   
    ...
    int thread_per_block = 256;
    int blocks = (total_blocks * 8 + thread_per_block - 1) / thread_per_block;
    idct_kernel<<<blocks, thread_per_block>>>(input.data().get(), total_blocks * 8, scale);
