...
float *MallocPlaneFloat(int width, int height, int *pStepBytes)
{
    ...
    *pStepBytes = ((int)ceil((width*sizeof(float))/16.0f))*16;
    ...
    printf("in MallocPlaneFloat: pStepBytes %d sizeof(float) %ld\n", *pStepBytes, sizeof(float));
...
* \return Status code
*/
int PreLoadBmp(const char *FileName, int *Width, int *Height)
{
...
* \return None
*/
void LoadBmpAsGray(const char *FileName, int Stride, ROI ImSize, byte *Img)
{
    BMPFileHeader FileHeader;
    BMPInfoHeader InfoHeader;
    FILE *fh;
