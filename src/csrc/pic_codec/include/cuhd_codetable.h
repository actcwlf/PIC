/*****************************************************************************
 *
 * CUHD - A massively parallel Huffman decoder
 *
 * released under LGPL-3.0
 *
 * 2017-2018 André Weißenberger
 *
 *****************************************************************************/

#ifndef CUHD_CODETABLE_
#define CUHD_CODETABLE_

#include "cuhd_constants.h"
#include "cuhd_definitions.h"

#include <memory>

namespace cuhd {
    struct CUHDCodetableItemSingle {
        BIT_COUNT_TYPE num_bits;
        SYMBOL_TYPE symbol;
    };

    struct CUHDCompactCodeTableItem {
        uint16_t cum_run_length;
        CUHDCodetableItemSingle item;
    };

    class CUHDCodetable {
        public:
            CUHDCodetable(size_t num_entries, int max_codeword_length);

            size_t get_size();
            size_t get_num_entries();
            size_t get_max_codeword_length();
            
            CUHDCodetableItemSingle* get();
            CUHDCompactCodeTableItem* get_compact_table();
            
        // private:
            
            // total number of rows
            size_t size_;

            // actual number of items
            size_t num_entries_;

            cuhd_buf(CUHDCodetableItemSingle, table_);
            cuhd_buf(CUHDCompactCodeTableItem, compact_table_);

            int m_max_codeword_length;
    };
}
#endif /* CUHD_CODETABLE_H_ */

